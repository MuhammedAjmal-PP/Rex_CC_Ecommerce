...
from catalog.models import Product, ProductVariant, ProductImage, InventoryLog
    ...
    if request.method == "POST":
        ...
        if variantform.is_valid() and imageformset.is_valid():
            ...
            if len(valid_images) < 3:
                ...
                return render(
                    request, "catalog/admin/product/variant_form.html", context
                )
            if primary_count != 1:
            ...
            if variant.stock > 0:
                InventoryLog.objects.create(
                    ...
                    note="Initial stock set on variant creation",
                    actor=request.user,
                )
            for form in imageformset:
                if form.cleaned_data and form.cleaned_data.get("image"):
...
def variant_edit(request, product_id, variant_id):
    ...
    old_stock = variant.stock  # capture before form overwrites
    if request.method == "POST":
        ...
        if variantform.is_valid() and imageformset.is_valid():
            ...
            if primary_count != 1:
                messages.error(
                    request,
                    "Exactly one image must be set as primary. Updates were not saved.",
                )
                context = {
                    "variantform": variantform,
                    "imageformset": imageformset,
            ...
            if stock_change != 0:
                InventoryLog.objects.create(
                    ...
                    reason="ADMIN_ADJUSTMENT",
                    note=f"Stock updated via variant edit (was {old_stock}, now {new_stock})",
                    actor=request.user,
                )
            for form in imageformset:
