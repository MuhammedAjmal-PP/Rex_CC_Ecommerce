...
class OfferForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "offer_type": forms.Select(
                attrs={"class": "form-control", "id": "id_offer_type"}
            ),
    ...
    def _date_changed(self, field_name):
        ...
        if not (self.instance and self.instance.pk):
            return True
        saved = (
            Offer.objects.filter(pk=self.instance.pk)
    ...
    def clean(self):
        ...
        if offer_type == "PRODUCT":
            if not products:
                self.add_error(
                    "products", "Product offer must have at least one product."
                )
            ...
            if not categories:
                self.add_error(
                    "categories", "Category offer must have at least one category."
                )
