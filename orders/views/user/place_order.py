...
def place_order_view(request):
    ...
    shipping_address = get_object_or_404(
        Address, id=address_id, user=request.user, is_active=True,
    )
    ...
    adjusted = recalculate_with_coupon(
        summary["sub_total"], summary["shipping_fee"],
        settings.GST_RATE, coupon_discount,
    )
    ...
    with transaction.atomic():
        ...
        if payment_method == "RAZORPAY":
            ...
            txn = create_transaction(
                ...
                method="RAZORPAY", amount=order.grand_total,
                status="PENDING", content_object=order,
        ...
        # ── COD / WALLET: full order (items + stock) ──
            ...
            for item in cart_items:
                ...
                order_item = OrderItem.objects.create(
                    ...
                    price=packed_prices.get(item.product_variant_id, locked_variant.price),
            ...
            if payment_method == "WALLET":
                ...
                txn = create_transaction(
                    ...
                    status="PAID", content_object=order,
                ...
                txn = create_transaction(
                    user=request.user, txn_type="ORDER_PAYMENT",
                    method="COD", amount=order.grand_total,
                    status="PENDING", content_object=order,
                    note=f"COD payment for order {order.order_number}",
                )

        # ── Common: coupon + cart cleanup ──
        if coupon_obj:
